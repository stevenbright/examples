...
/* this method is like the demo_with_find_interface_method but resembles autogenerated code */
static void demo_with_find_interface() {
    ...
    for (i = 0; i < 4; ++i) {
        ...
        for (j = 0; j < ITERATIONS; ++j) {
            ...
            int result = ((Obj_foo_ptr) objItable->methods[I_OBJ_IDX_FOO])(myObj);
            assert(result == 0);
        }
        gettimeofday(&stop, NULL);
...
/* this method is like the demo_with_find_interface but unoptimized one */
static void demo_with_find_interface_unopt() {
    struct MyObj * myObj = createMyObj();
    int i;
    ...
    for (i = 0; i < 4; ++i) {
        ...
        for (j = 0; j < ITERATIONS; ++j) {
            /* equivalent of java's myObj.foo() */
            struct Itable * objItable = find_interface(myObj->clazz, I_OBJ_ID);
            int result = ((Obj_foo_ptr) objItable->methods[I_OBJ_IDX_FOO])(myObj);
            assert(result == 0);
        }
        gettimeofday(&stop, NULL);
