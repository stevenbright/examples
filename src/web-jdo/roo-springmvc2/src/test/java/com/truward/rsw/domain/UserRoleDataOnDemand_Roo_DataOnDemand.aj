...
privileged aspect UserRoleDataOnDemand_Roo_DataOnDemand {
    ...
    public UserRole UserRoleDataOnDemand.getNewTransientUserRole(int index) {
        com.truward.rsw.domain.UserRole obj = new com.truward.rsw.domain.UserRole();
    ...
    public void UserRoleDataOnDemand.init() {
        data = com.truward.rsw.domain.UserRole.findUserRoleEntries(0, 10);
        if (data == null) throw new IllegalStateException("Find entries implementation for 'UserRole' illegally returned null");
        if (!data.isEmpty()) {
            return;
        }
        
        data = new java.util.ArrayList<com.truward.rsw.domain.UserRole>();
        for (int i = 0; i < 10; i++) {
            com.truward.rsw.domain.UserRole obj = getNewTransientUserRole(i);
            obj.persist();
            obj.flush();
            data.add(obj);
        }
